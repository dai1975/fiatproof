...
pub fn base58_serialize(bytes:&[u8]) -> String {
   BASE58.serialize(bytes)
}
pub fn base58_deserialize(s:&str) -> ::Result<Vec<u8>> {
   BASE58.deserialize(s)
}

pub fn base58check_serialize(bytes:&[u8], version:&[u8]) -> String {
   ...
   BASE58.serialize(v.as_slice())
}

pub fn base58check_deserialize(s:&str) -> ::Result<Vec<u8>> {
   let mut v = try!(BASE58.deserialize(s));
   let len0 = v.len() - 4;
   let mut check = [0u8; 32];
   {
      use ::crypto::Digest;
...
mod tests {
   use ::bitcoin::utils::base58_serialize;
   use ::bitcoin::utils::base58_deserialize;
   use ::bitcoin::utils::base58check_serialize;
   use ::bitcoin::utils::base58check_deserialize;
   ...
   fn test_serialize_b58() {
      ...
      let result = base58_serialize(&data);
   ...
   fn test_deserialize_b58() {
      let data:&[u8] = &[0x10, 0xc8, 0x51, 0x1e]; //0x10c8511e = 281563422
      let enc = "Rt5zm"; // 281563422 = 22*58^4 + 51*58^4 + 4*58^4 + 57*58^4 + 44*58^0
      let result = base58_deserialize(enc);
   ...
   fn test_serialize_b58check() {
      ...
      let result = base58check_serialize(&data, &[0u8]);
      assert_eq!(enc, result);
   }
   ...
   fn test_deserialize_b58check() {
      let data:&[u8] = &[0x10, 0xc8, 0x51, 0x1e]; //0x10c8511e = 281563422
      let enc = "13op3it3Aaiu";
      let result = base58check_deserialize(enc);
      assert_matches!(result, Ok(_));
