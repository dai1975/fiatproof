...
impl ::std::fmt::Display for UInt256 {
   fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
      match ::ui::BitcoinSerializer::serialize(self, &()).map(|b| ::utils::b2h_rev(b)) {
...
fn test_str() {
   ...
   let uint256 = ::ui::BitcoinDeserializer::hex_to_uint256(s).unwrap();
...
fn test_serialize() {
   ...
   let octets = ::ui::BitcoinSerializer::serialize(&v, &()).unwrap();
...
fn test_deserialize() {
   ...
   let r = ::ui::BitcoinDeserializer::deserialize(data, &(), &mut v);
