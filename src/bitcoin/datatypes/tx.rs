use super::{UInt256, TxIn, TxOut, LockTime};

#[derive(Debug,Default,Clone)]
pub struct Tx {
   pub version:  i32,
   pub ins:      Vec<TxIn>,
   pub outs:     Vec<TxOut>,
   pub locktime: LockTime,
}

const TRANSACTION_CURRENT_VERSION:i32 = 1i32;

impl Tx {
   pub fn new_null() -> Self {
      Tx {
         version:  TRANSACTION_CURRENT_VERSION,
         ins:      Vec::<TxIn>::new(), 
         outs:     Vec::<TxOut>::new(), 
         locktime: LockTime::NoLock,
      }
   }
   pub fn is_coin_base(&self) -> bool {
      self.ins.len() == 1 && self.ins[0].prevout.is_null()
   }
   pub fn is_null(&self) -> bool {
      self.ins.len() == 0 && self.outs.len() == 0
   }
   pub fn get_hash(&self) -> ::Result<UInt256> {
      let b = ::ui::BitcoinSerializer::serialize(self, &())?;
      let b = ::ui::DIGEST.create_dhash256().u8_to_box(b);
      Ok(UInt256::new(b.as_ref()))
   }
}

impl ::std::fmt::Display for Tx {
   fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
      write!(f, "Tx(ver={}, ins={}, outs={}, locktime={:?})", self.version, self.ins.len(), self.outs.len(), self.locktime)
   }
}

use ::iostream::{ WriteStream, ReadStream };
use ::bitcoin::serialize::{
   Serializer as BitcoinSerializer,
   Serializee as BitcoinSerializee,
   Deserializer as BitcoinDeserializer,
   Deserializee as BitcoinDeserializee,
};
impl BitcoinSerializee for Tx {
   type P = ();
   fn serialize(&self, _p:&Self::P, e:&BitcoinSerializer, ws:&mut WriteStream) -> ::Result<usize> {
      let mut r:usize = 0;
      r += try!(e.serialize_i32le(ws, self.version));
      r += try!(e.serialize_var_array(&(), ws, self.ins.as_slice(), ::std::usize::MAX));
      r += try!(e.serialize_var_array(&(), ws, self.outs.as_slice(), ::std::usize::MAX));
      r += try!(self.locktime.serialize(&(), e, ws));
      Ok(r)
   }
}
impl BitcoinDeserializee for Tx {
   type P = ();
   fn deserialize(&mut self, _p:&Self::P, d:&BitcoinDeserializer, rs:&mut ReadStream) -> ::Result<usize> {
      let mut r:usize = 0;
      r += try!(d.deserialize_i32le(rs, &mut self.version));
      r += try!(d.deserialize_var_array(&(), rs, &mut self.ins, ::std::usize::MAX));
      r += try!(d.deserialize_var_array(&(), rs, &mut self.outs, ::std::usize::MAX));
      r += try!(self.locktime.deserialize(&(), d, rs));
      Ok(r)
   }
}

#[test]
fn test_deserialize_transaction() {
   use super::Tx;
   let hex = "";

   let tx = ::ui::BitcoinDeserializer::hex_to_tx(hex);
   assert_matches!(tx, Ok(_));
   let tx = tx.unwrap();
   assert_eq!(tx.version, 1);
   assert_eq!(tx.ins.len(), 2);
   assert_eq!(tx.outs.len(), 2);
   assert_eq!(tx.locktime.is_no_lock(), true);

   assert_eq!(::ui::BitcoinSerializer::uint256_to_hex(&tx.ins[0].prevout.txid).unwrap(), "4d6da9420d472b6b52c36eee132d87448bf160d8839a58afdd2add6f6adfc8d8");
   assert_eq!(tx.ins[0].prevout.n, 0);
   assert_eq!(tx.ins[0].is_sequence_final(), true);
   assert_eq!(::ui::BitcoinSerializer::script_to_hex(&tx.ins[0].script_sig).unwrap(), "");

   assert_eq!(::ui::BitcoinSerializer::uint256_to_hex(&tx.ins[1].prevout.txid).unwrap(), "b220a19ba645c021f0cf501435fd6c3b4db960325d0834612612a5684ffab32a");
   assert_eq!(tx.ins[1].prevout.n, 0);
   assert_eq!(tx.ins[1].is_sequence_final(), true);
   assert_eq!(::ui::BitcoinSerializer::script_to_hex(&tx.ins[1].script_sig).unwrap(), "");

   assert_eq!(tx.outs[0].value, 39000000);
   assert_eq!(::ui::BitcoinSerializer::script_to_hex(&tx.outs[0].script_pubkey).unwrap(), "76a914a3d89c53bb956f08917b44d113c6b2bcbe0c29b788ac");
   assert_eq!(tx.outs[1].value, 155000000);
   assert_eq!(::ui::BitcoinSerializer::script_to_hex(&tx.outs[1].script_pubkey).unwrap(), "76a91408338e1d5e26db3fce21b011795b1c3c8a5a5d0788ac");
}

#[test]
fn test_serialize_transaction() {
   use super::{UInt256, Script, TxIn, TxOutPoint};

   let mut tx = Tx::new_null();
   tx.ins.push(TxIn {
      prevout: TxOutPoint {
         txid: ::ui::BitcoinDeserializer::hex_to_uint256("4d6da9420d472b6b52c36eee132d87448bf160d8839a58afdd2add6f6adfc8d8").unwrap(),
         n:0
      },
      script_sig: ::ui::BitcoinDeserializer::hex_to_script("").unwrap(),
      sequence: 0xFFFFFFFFu32,
   } );
   tx.ins.push(TxIn {
      prevout: TxOutPoint {
         txid: ::ui::BitcoinDeserializer::hex_to_uint256("b220a19ba645c021f0cf501435fd6c3b4db960325d0834612612a5684ffab32a").unwrap(),
         n:0
      },
      script_sig: ::ui::BitcoinDeserializer::hex_to_script("").unwrap(),
      sequence: 0xFFFFFFFFu32,
   } );
   tx.outs.push(TxOut {
      value: 39000000,
      script_pubkey: ::ui::BitcoinDeserializer::hex_to_script("76a914a3d89c53bb956f08917b44d113c6b2bcbe0c29b788ac").unwrap(),
   });
   tx.outs.push(TxOut {
      value: 155000000,
      script_pubkey: ::ui::BitcoinDeserializer::hex_to_script("76a91408338e1d5e26db3fce21b011795b1c3c8a5a5d0788ac").unwrap(),
   });

   assert_eq!(::ui::BitcoinSerializer::tx_to_hex(&tx).unwrap(), "");
   
   assert_eq!(::ui::BitcoinSerializer::tx_to_dhash256hex(&tx).unwrap(), "9021b49d445c719106c95d561b9c3fac7bcb3650db67684a9226cd7fa1e1c1a0");
}
